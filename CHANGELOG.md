...
## Unreleased
### Added
- Added the option `--with-dependencies` / `-w` to the `fixture:run:group` command
  - This command recursively runs all fixtures that are required by the given fixture even if they are not in the same group
- Added new method to the fixture trait: `runFixtureGroupWithDependencies()`
